...
def test_attention_triton_01(
    ...
    with torch.cuda.device(q.device.index):
        out = attention_gym.sparge_sage1_triton(
            q, k, v, tensor_layout="HND", is_causal=causal
        )
        out_ref = spas_sage_attn_meansim_cuda(
            q, k, v, tensor_layout="HND", is_causal=causal
        )
    ...
    print(f"sparge_sage1_triton vs sparge_sage1_cuda abs_diff : {abs_diff}")
    print(f"sparge_sage1_triton vs sparge_sage1_cuda max_diff : {max_diff}")

    print("show tensor")
    print("sparge_sage1_triton")
    print(out_ref[0][0][0][:10])
    print("sparge_sage1_cuda")
    print(out_ref[0][0][0][:10])


if __name__ == "__main__":
    pytest.main(["test_sparge_sage1.py", "-s"])
