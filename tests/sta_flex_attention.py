...
def sta_flex_attention(
    ...
    # only support layout for "HND" `
    # tensor_layout` is "HND": ``[batch_size, num_qo_heads, qo_len, head_dim]``.

    # every head shared the same window size
    torch.cuda.set_device(query.device)

    device = query.device

    block_mask = get_sliding_tile_attention_mask(
